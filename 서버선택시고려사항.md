...
- 스레드 풀에서 '작업 큐'라는 큐를 도입하여 작업 처리 요청이 많아져도 작업 큐에서 대기하기 때문에 요청마냥 스레드의 개수를 늘리지 않고 스레드에 전체 개수는 일정하며 성능이 저하되지 않는다.
...
- Request 이후 바로 Return 되고 다른 작업을 수행하면서 Respose의 준비 여부를 체크해 Response를 처리한다. Response의 주기적 체크 방식을 Polling이라는 하는데 작업이 완료대기 전까지 지속적으로 호출하는 리소스를 사용해야 하므로 시스템 효율성이 떨어질 수 있다.
...
출처: https://jeong-pro.tistory.com/204 [기본기를 쌓는 정아마추어 코딩블로그:티스토리]
### [Spring] 쓰레드 관점에서 본 Spring MVC HTTP 요청과정
출처: https://livenow14.tistory.com/59 [경험의 연장선:티스토리]
